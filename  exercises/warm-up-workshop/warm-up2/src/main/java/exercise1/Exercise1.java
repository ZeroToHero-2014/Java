...
public class Exercise1 {
    ...
    public List<String> iteratingOverKeys(){
        ...
        for(String element:countries.keySet()){
            if(element.startsWith("R")){
                seekingCountries.add(element);
            }
        }
    ...
    public List<String> iteratingOverEntries(){
        ...
        for(Map.Entry<String, String> element:countries.entrySet()){
            if(element.getKey().startsWith("R")){
                seekingCountries.add(element.getKey().toLowerCase());
            }
        }
    ...
    public String iteratingOverValues(){
        ...
        for(String element:countries.values()){
            if(element.length()>numar){
                numar=element.length();
                seekingCapital=element;
            }
        }
