...
public class Exercise1 {
    ...
    public List<String> iteratingOverKeys(){
        ...
        for(String country : mySet){
            if(country.startsWith("R") || country.startsWith("r"))
    ...
    public List<String> iteratingOverEntries(){
        ...
        Set<Map.Entry<String,String>> mySet2 = countries.entrySet();

        for(Map.Entry<String,String> country : mySet2){
            if(country.getKey().startsWith("R"))
                seekingCountries.add(country.getKey().toLowerCase());
        }
    ...
    public String iteratingOverValues(){
        ...
        int max = seekingCapital.length();

        Collection<String> myColl = countries.values();

        for(String value : myColl){
            if(value.length() > max){
                max = value.length();
