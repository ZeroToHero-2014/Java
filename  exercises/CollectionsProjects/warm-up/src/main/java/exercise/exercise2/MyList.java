...
public class MyList<Integer> extends ArrayList<Integer> {
    ...
    public boolean addAll(Collection integers) {
        for(Object val : integers) {
            if(!this.add((Integer)val)) {
                return false;
            }
        }
        return true;
    ...
    @Override
    public boolean addAll(int i, Collection integers) {
        //return super.addAll(i, integers);
        for(Integer val : (Collection<Integer>)integers) {
            this.add(i, val);
            i++;
        }
        return true;
